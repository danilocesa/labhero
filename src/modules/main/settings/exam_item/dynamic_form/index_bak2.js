...
class DynamicForm extends React.Component {
	...
	componentDidUpdate(prevProps) {
		...
		if(examId !== prevProps.examId && examItemValue) {
			console.log('dynamic input set field value');
	...
	remove = (key, fields, callbackRemove) => {
		const { checkedKey } = this.state;
    ...
    //   </Form.Item>
		...
		return (
			<div className="exam-item-dyna-form">
				{/* { OptionFormItems } */}
				<Form.List name="dynamicItems" ref={this.test}>
					{(fields, { add, remove }) => {
						return (
							<div>
											...
											extra={(
												<>
													...
													<MinusCircleOutlined
														className="dynamic-delete-button"
														// onClick={() => this.remove(field.key)}
														onClick={() => this.remove(field.key, fields, remove)}
													/>
												</>	
											)}
										>
